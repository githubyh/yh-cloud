...
public class ConcurrentMapWithMap {
    ...
    public static long ConcurrentMapIncrease2(String word) {
        Long oldValue, newValue;
        ...
        concurrentMapWordCounts.put(word, newValue);
    ...
    public static void concurrentWordCount() throws InterruptedException, ExecutionException {
        new Thread(new Runnable() {
            public void run() {
                int count = 0;
                while (count++ < 10000)
                    logger.info("concurrentWordCount num is " + ConcurrentMapWithMap.ConcurrentMapIncrease2("work"));
            }
        }).start();
        new Thread(new Runnable() {
            public void run() {
 ...
 /*
       ...
       ClassLoader cs = ConcurrentMapWithMap.class.getClassLoader();
        System.out.println(cs);

        ClassLoader cs1 = cs.getParent();
        System.out.println(cs1);

        ClassLoader cs2 = cs1.getParent();
